...
Lighter 主网 ↔ Lighter Robinhood ↔ SoDEX 永续价差监控与套利（P1 + P1.5 + 实盘闭环）。默认 `monitor_only: true`、`scan.enabled: true` 三所两两扫描；可选 `execution.enabled: true` 走格子 + 定仓 + 执行（paper 或经 `exchange_bridge` 实盘）。详见 [docs/实盘验证.md](docs/实盘验证.md)。
...
| [docs/实盘验证.md](docs/实盘验证.md) | **小额实盘**：exchange_bridge + live-test CLI |
...
4. 小额实盘：先 `pip install -r scripts/requirements-exchange.txt` 与 `cargo run --bin live-test -- account lighter`，再设 `paper_trading: false`、`sizing.mode: fixed`。页面：`http.enabled: true` 后访问 `http://127.0.0.1:8090/`。
