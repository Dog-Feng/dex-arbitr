// sodex_bridge：对齐 internal/exchange/sodex.go + 官方 sodex-go-sdk-public。
// Rust/Python exchange_bridge 通过 stdin JSON 调用：
//
...
func main() {
	...
	data, err := dispatch(context.Background(), req)
	if err != nil {
		respond(false, nil, err.Error())
		return
	}
...
func dispatch(ctx context.Context, req request) (any, error) {
	venue, err := loadVenue(req.VenueYAML)
	if err != nil {
		return nil, err
	}
	...
	case "place":
		return placeOrder(ctx, client, accountID, venue, req.Params)
	case "cancel":
		return cancelOrder(ctx, client, accountID, req.Params)
	case "order_status":
		return orderStatus(ctx, client, accountID, req.Params)
	default:
		return nil, fmt.Errorf("unknown cmd %q", req.Cmd)
	}
}

func loadVenue(path string) (venueFile, error) {
	b, err := os.ReadFile(path)
	if err != nil {
		return venueFile{}, err
	}
	var v venueFile
	...
	if v.PrivateKey == "" {
		return venueFile{}, fmt.Errorf("sodex private_key missing in %s", path)
	}
	if v.APIKeyName == "" {
		return venueFile{}, fmt.Errorf("sodex api_key_name missing in %s", path)
	}
...
func readAccountAddress(v venueFile, c *sodexclient.Client) string {
...
func eip712Chain(v venueFile) uint64 {
	if v.EIP712Chain > 0 {
...
func configuredAccountID(v venueFile) uint64 {
	if v.AccountID > 0 {
...
func connect(ctx context.Context, v venueFile) (*sodexclient.Client, uint64, string, error) {
...
func placeOrder(ctx context.Context, c *sodexclient.Client, accountID uint64, v venueFile, params map[string]any) (map[string]string, error) {
