...
面板：套利配置页「交易所持仓」**一行一个已加载所**（yaml `venues` 有几个就几行）。列：交易所、点差中枢、交易量、**签名→确认**（`sign+send=合计ms · 全链路`；Lighter / SoDEX / Entropy 同一口径，见 [新增DEX对齐清单.md](新增DEX对齐清单.md) §4.9；未下单 `—`）、持仓。顶栏「交易所持仓」计数仍是原始仓条数，不是所的行数。
