...
面板：套利配置页「交易所持仓」**一行一个已加载所**（yaml `venues` 有几个就几行）。列：交易所、点差中枢、交易量（本次进程该所成交名义 = 成交量 × 成交价，开平都累计）、持仓（该所原始仓汇总）。顶栏「交易所持仓」计数仍是原始仓条数，不是所的行数。
...
未满点差窗：空仓不开（监控「点差 n0/cap n1/cap」）。有仓时暂时只用 \(F\)（\(C=0\)）以便还能减格。yaml 静态 Δ 不含 C；匹配瞬间日志 `round_trip_spread` 常为 0，满窗后才带上。
